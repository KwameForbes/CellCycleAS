...
go <- subset(go,select = c(1,2))
go <-unique(go)
go2 <- AnnotationDbi::select(GO.db, go$GO, c("TERM","DEFINITION"), "GOID")
counting <- as.data.frame(count(go2, TERM))
counting <- counting[order(-counting$n),]
head(counting,10)
